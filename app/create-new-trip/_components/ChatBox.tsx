...
function ChatBox({ onTripGenerated }: { onTripGenerated?: (trip: TripInfo) => void }) {
  ...
  const onSend = async (inputValue?: string, forceFinal?: boolean) => {
    ...
    try {
      const shouldUseFinal = typeof forceFinal === 'boolean' ? forceFinal : isFinal
      ...
      const assistantText =
      String(result?.data?.resp ?? 'Sorry, I could not generate a response.')
      ...
      if(shouldUseFinal){
        ...
        console.log('Trip detail saved:', savedTrip)
  ...
  const RenderGenerativeUI = (ui: string) => {
    const normalizedUi = ui?.trim().toLowerCase().replace(/\s+/g, "");
    ...
    if (normalizedUi === "budget") {
      ...
      return <FinalTripUi viewTrip={()=>console.log()}
  ...
  useEffect(()=>{
    ...
    if(lastMsg?.ui==="final" && !finalRequestTriggeredRef.current){
      finalRequestTriggeredRef.current = true
      setFinalRequestTriggered(true)
      setIsFinal(true)
      setUserInput("Ok,Great! I am ready to generate the final trip plan.")
  ...
  return (
    <div className='flex flex-col h-[80vh] '>
      {messages.length === 0 && (
        <EmptyBoxState onSelectOption={(value: string) => onSend(value)} />
      )}
      {/* Display messages */}
          ...
          return msg.role === 'user' ? (
            ...
            <div key={index} className='flex justify-start mt-2'>
              <div className='max-w-lg bg-gray-100 text-black px-4 py-2 rounded-lg'>
                {msg.content}
                {/* Only render the interactive selector on the most recent
                    assistant message. Older messages keep their text but
                    lose the clickable widget, so a stale/earlier selector
                    can never be clicked again and desync the conversation
                    order (this was the root cause of the "wrong order"
                    behavior). */}
                {isLastMessage && !loading && RenderGenerativeUI(msg.ui ?? '')}
              </div>
            </div>
          )
        })}
